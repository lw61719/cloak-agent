...
Provider 和模型由 `.env` 决定。网页可以查看实时工具轨迹、取消运行、复制或导出 Markdown 结果，并在“最近任务”中回看或恢复 URL、模式、任务和最大步骤；最多保留 20 条进程内记录，服务重启后清空。API key 只从服务器 `.env` 读取，不会返回给浏览器。Web 模式仍默认拒绝付款、删除、发布、上传和敏感字段输入，同时阻止本机及私网目标；按 `Ctrl/Command + K` 可以快速聚焦网址输入框。
