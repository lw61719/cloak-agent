...
然后打开 [http://127.0.0.1:8000](http://127.0.0.1:8000)。网页只需要输入 URL，后端会使用固定提示词提取页面准确标题、主要内容、关键事实和原始地址，并自动从 URL 生成域名白名单。Provider 和模型由 `.env` 决定。网页可以查看实时工具轨迹、取消运行、复制或导出 Markdown 结果，并在“最近任务”中回看或重新使用最多 20 条运行记录；按 `Ctrl/Command + K` 可以快速聚焦网址输入框。历史记录保存在当前服务进程内，服务重启后清空。API key 只从服务器 `.env` 读取，不会返回给浏览器；Web 模式默认拒绝付款、删除、发布、上传和敏感字段输入，同时阻止本机及私网目标。
